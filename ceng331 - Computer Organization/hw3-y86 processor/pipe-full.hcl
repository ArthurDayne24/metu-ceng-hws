...
#
#  Additional information is given in below HCL computattions
# as comments starting with "ADDED : "
#
# Stages for computation of IIADDQ 
#
# Fetch:     icode:ifun <- M1[PC]  # Note: ifun is 0
#            rA:rB <- M1[PC+1]     # Note: rA is RNONE
#            valC  <- M8[PC+2]     # Read immediate
#            valP  <- PC + 10      # Instruction's size is 10 bytes 
#
# Decode:    valB <- R[rB]         # Read register
#
# Execute:   valE <- valB + valC   # Set aluA to valC and aluB to valB
#                                  #   and set do computation of valE
#            Set CC (ZF, SF, OF)   # Set condition flags according to valB
#                                  #   and valC. It also helps us to remove
#                                  #   additional andq instructions in ncopy.ys
#                                  #   to compare two numbers
#
# Memory:    < No operation here >
#
# WriteBack: R[rB]  <- valE        # Set dest register as rB, write result to
#                                  #   rB and complete computation
#        
...
bool instr_valid = f_icode in 
        # ADDED: Define IIADDQ as a valid instruction
...
bool need_regids =
        # ADDED : IIADDQ needs a destination register rB
	f_icode in { IRRMOVQ, IOPQ, IIADDQ, IPUSHQ, IPOPQ, 
		     IIRMOVQ, IRMMOVQ, IMRMOVQ };

# Does fetched instruction require a constant word?
bool need_valC =
        # ADDED : IIADDQ needs a constant(immediate) as parmeter
...
## Should the condition codes be updated?
        # ADDED : IIADDQ sets CC's as it is kind of an arithmetic operation
